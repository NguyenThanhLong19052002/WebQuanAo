    ...
    <figure class="product">
        ...
        <div class="image">
            <?php 
				...
				if(count($imgs)>0){
					$img = $imgs[0]['Url']; ?>
            <img src="{!! url('/images/') !!}<?php echo  '/'.$img?>" class="clickDetail"
                onclick="detailProduct(<?php echo $value->id; ?>)">
            <?php 
				}  ?>
            <a data-id='<?php echo $value->id; ?>' data-name="<?php echo $value->Name; ?>" class="add-to-cart">Mua
                hàng</a>
        </div>
        <!-- Name and price -->
        <figcaption class="clickDetail" onclick="detailProduct(<?php echo $value->id; ?>)">
            <!-- name prodyct -->
            <h3 class="nameProductHome"
...
<script type="text/javascript" src="{!! url('/js/Home/product.js') !!}"></script>
<?php 
}else {
