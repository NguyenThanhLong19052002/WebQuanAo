...
var listDataTable = [
    ...
    <?php foreach ($listProduct as $key => $value): ?> {
        "data": {
            ...
            'STT': '<?php echo $key+1; ?>',
            <?php $listPicture = $value->Pictures()->get() ?> 'Picture': '<?php foreach ($listPicture as $key => $valuePicture): ?><img class="img-product" src="{!! url("/images/") !!}/<?php echo $valuePicture->Url; ?>"><?php endforeach ?>',
            'Name': "<?php echo $value->Name; ?>",

            <?php $Catrgory = $value->ProductCategory()->get(); ?> 'ProductCategory': "<?php echo isset($Catrgory[0]->Name) ? $Catrgory[0]->Name : ''; ?>",
...
<!-- table sort -->
<script type="text/javascript" src="{!! url('/js/table-sort/PagingTable.js') !!}"></script>
<script type="text/javascript" src="{!! url('/js/ManageProduct/_search.js') !!}"></script>